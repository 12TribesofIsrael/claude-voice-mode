...
_TECH_JARGON = {
    r"\bAPI\b": "interface",
    r"\bCLI\b": "command line tool",
    r"\bGUI\b": "graphical interface",
    r"\bJSON\b": "data format",
    r"\bSQL\b": "database language",
    r"\bHTTP\b": "web protocol",
    r"\bHTTPS\b": "secure web protocol",
    r"\bURL\b": "web address",
    r"\bURI\b": "web address",
    r"\bRESTful\b": "web service style",
    r"\bREPL\b": "interactive shell",
    r"\bIDEentifier\b": "unique identifier",
    r"\bAPI endpoint\b": "web address for a service",
    r"\bpayload\b": "data being sent",
    r"\bquery string\b": "search parameters",
    r"\brequest body\b": "data sent with a request",
    r"\bresponse\b": "answer from a service",
    r"\bassert\b": "check that something is true",
    r"\bbool\b": "true or false value",
    r"\bstring\b": "text",
    r"\binteger\b": "whole number",
    r"\bfloat\b": "decimal number",
    r"\barray\b": "list",
    r"\bobject\b": "thing with properties",
    r"\bmethod\b": "function",
    r"\battribute\b": "property",
    r"\bencapsulation\b": "hiding details",
    r"\bpolymorphism\b": "same name, different behavior",
    r"\binheritance\b": "getting properties from a parent",
    r"\binstanciate\b": "create a copy",
    r"\brefactor\b": "reorganize code",
    r"\bdebug\b": "find and fix problems",
    r"\biterative\b": "step by step",
    r"\brecursive\b": "doing something by calling itself",
    r"\basync\b": "happening at its own pace",
    r"\bsynchronous\b": "one after the other",
    r"\bconcurrency\b": "doing multiple things at once",
    r"\bdeployment\b": "putting code online",
    r"\bmiddleware\b": "software in the middle",
    r"\bframework\b": "toolbox of code",
    r"\blibrary\b": "collection of code",
    r"\bmodule\b": "piece of code",
    r"\bpackage\b": "bundled code",
    r"\bdependency\b": "code you need",
    r"\bversion control\b": "tracking changes",
    r"\brepository\b": "code storage",
    r"\bcommit\b": "save a change",
    r"\bmerge\b": "combine changes",
    r"\brolebase\b": "access control by job",
    r"\bauthentication\b": "proving who you are",
    r"\bauthorization\b": "deciding what you can do",
    r"\btoken\b": "permission badge",
    r"\bsession\b": "a visit",
    r"\bcookie\b": "stored information",
    r"\bcache\b": "fast storage",
    r"\btransmit\b": "send",
    r"\bencode\b": "change the format",
    r"\bdecode\b": "change back from format",
    r"\ncompress\b": "make smaller",
    r"\bdecompress\b": "make bigger",
    r"\bserialize\b": "turn into text",
    r"\bdeserialize\b": "turn back into objects",
    r"\bvalidation\b": "checking it's right",
    r"\bsanitization\b": "cleaning up input",
    r"\bscaling\b": "growing bigger",
    r"\blockfile\b": "slow file storage",
    r"\bbucket\b": "storage folder",
    r"\bqueue\b": "waiting line",
    r"\bPubSub\b": "announcement system",
    r"\bstream\b": "flow of data",
    r"\blatency\b": "delay time",
    r"\bthroughput\b": "speed",
    r"\bbandwidth\b": "data capacity",
    r"\bfirewall\b": "network gate",
    r"\bproxy\b": "middleman",
    r"\bgateway\b": "entry point",
    r"\bloadbalancer\b": "traffic distributor",
    r"\bcontainer\b": "packaged program",
    r"\bimage\b": "program recipe",
    r"\borchestration\b": "managing many pieces",
    r"\bcluster\b": "group of computers",
    r"\bnode\b": "computer in a group",
    r"\bshard\b": "data slice",
    r"\breplica\b": "copy",
    r"\bbackup\b": "safety copy",
    r"\bsnapshot\b": "point in time copy",
    r"\bindex\b": "quick lookup list",
    r"\bquery\b": "question asked",
    r"\bfull text search\b": "finding words",
    r"\bpagination\b": "splitting into pages",
    r"\bfiltering\b": "picking what you want",
    r"\baggregation\b": "combining data",
    r"\bsort\b": "putting in order",
    r"\bgroup by\b": "organizing by type",
    r"\bjoin\b": "combining tables",
    r"\bmigration\b": "moving data",
    r"\bschema\b": "data structure",
    r"\btransaction\b": "grouped action",
    r"\batomic\b": "all or nothing",
    r"\bscheduler\b": "automatic runner",
    r"\bcron\b": "timer",
    r"\bwebhook\b": "event notification",
    r"\bevent\b": "something that happened",
    r"\bemitter\b": "thing that sends events",
    r"\blistener\b": "thing that waits for events",
    r"\bpipeline\b": "series of steps",
    r"\bworkflow\b": "series of tasks",
    r"\btest coverage\b": "how much is tested",
    r"\bCI/CD\b": "automated building and deployment",
    r"\blinting\b": "checking code style",
    r"\btype checking\b": "verifying data types",
    r"\bdeadlock\b": "being stuck",
    r"\brace condition\b": "timing problem",
    r"\bmemory leak\b": "lost memory",
    r"\bbuffer overflow\b": "data spill",
    r"\bstack overflow\b": "too much nesting",
    r"\bsegmentation fault\b": "illegal memory access",
    r"\bGarbage collection\b": "automatic cleanup",
    r"\bheap\b": "memory pile",
    r"\bstack\b": "memory tower",
    r"\bpointer\b": "memory address",
    r"\breference\b": "link to something",
    r"\biterator\b": "thing that goes through a list",
    r"\bgenerator\b": "thing that makes a list",
    r"\bclosure\b": "function remembering its place",
    r"\bdecorator\b": "function wrapper",
    r"\binterface\b": "contract",
    r"\babstraction\b": "hiding details",
    r"\bconcreteimplementation\b": "real thing",
    r"\bfactory\b": "thing maker",
    r"\bsingleton\b": "only one copy",
    r"\bobserver\b": "watcher",
    r"\bmutex\b": "lock",
    r"\bsemaphore\b": "gate",
    r"\batomic operation\b": "unbreakable action",
    r"\bdispatching\b": "sending to handler",
    r"\brenderingpipe\b": "drawing process",
    r"\basynchronous\b": "out of order",
}


def simplify_to_laymans_terms(text):
    """Convert technical jargon to plain language using regex replacements.

    This is a basic implementation using common tech-to-plain mappings. For
    production use with more complex documents, consider integrating Claude's
    API for intelligent contextual simplification.
    """
    if not text or not text.strip():
        return text

    result = text
    for pattern, replacement in _TECH_JARGON.items():
